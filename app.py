import streamlit as st
import pandas as pd
import plotly.express as px
import numpy as np

# 1. CONFIGURAZIONE PAGINA
st.set_page_config(page_title="watch42 | Market Intelligence", layout="wide")

# 2. INJECT DEFINITIVE CSS (Focalizzato sull'allineamento Sidebar)
def inject_custom_css():
    st.markdown("""
        <style>
        /* Sfondo e Layout Generale */
        .main { background-color: #F8F9FC; }
        [data-testid="stSidebar"] {
            background-color: #FBFBFE !important;
            border-right: 1px solid #E5E7EB !important;
        }

        /* INTESTAZIONE SEZIONE (REPORTS / MENU) */
        .sidebar-header {
            font-size: 11px;
            font-weight: 700;
            color: #9CA3AF;
            letter-spacing: 1.5px;
            text-transform: uppercase;
            padding: 30px 25px 15px 25px;
            font-family: 'Inter', sans-serif;
        }

        /* FORZATURA ALLINEAMENTO A SINISTRA DEI PULSANTI */
        /* Questo blocca il centraggio di Streamlit */
        [data-testid="stSidebar"] .stButton {
            display: flex;
            justify-content: flex-start !important;
            width: 100%;
        }

        [data-testid="stSidebar"] .stButton > button {
            width: 100% !important;
            border: none !important;
            background-color: transparent !important;
            text-align: left !important; /* Allineamento testo a sinistra */
            padding: 12px 25px !important; /* Padding interno coerente */
            border-radius: 0px !important;
            color: #1F2937 !important;
            font-size: 15px !important;
            font-weight: 500 !important;
            display: flex !important;
            align-items: center !important;
            justify-content: flex-start !important; /* Forza l'icona a sinistra */
            gap: 15px !important;
            transition: all 0.2s ease;
        }

        /* Effetto Hover */
        [data-testid="stSidebar"] .stButton > button:hover {
            background-color: #F3F4F6 !important;
            color: #2E5BFF !important;
        }

        /* Stile Card area principale */
        .watch-card {
            background-color: #FFFFFF;
            padding: 24px;
            border-radius: 20px;
            box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.05);
            border: 1px solid #F3F4F6;
            margin-bottom: 20px;
        }
        </style>
    """, unsafe_allow_html=True)

inject_custom_css()

# 3. MOCK DATA
@st.cache_data
def get_mock_data():
    return pd.DataFrame({
        'Model': [f'Watch {i}' for i in range(25)],
        'Price': np.random.randint(800, 50000, 25),
        'PowerScore': np.random.randint(38, 80, 25),
        'Diameter': np.random.randint(34, 48, 25),
        'Thickness': np.random.randint(6, 18, 25),
        'Brand': np.random.choice(["Mido", "Rolex", "Cartier"], 25)
    })

data = get_mock_data()

# 4. SIDEBAR NAVIGATION (Layout Screenshot 14.43.19)
st.sidebar.markdown('<div class="sidebar-header">MENU</div>', unsafe_allow_html=True)

if 'menu' not in st.session_state:
    st.session_state.menu = "My Watches"

# Pulsanti con icone minimali e testo allineati a sinistra
if st.sidebar.button("⌚ My Watches"):
    st.session_state.menu = "My Watches"
if st.sidebar.button("📊 Pricing Intelligence"):
    st.session_state.menu = "Pricing Intelligence"
if st.sidebar.button("🗺️ Design Intelligence"):
    st.session_state.menu = "Design Intelligence"
if st.sidebar.button("📈 Market Intelligence"):
    st.session_state.menu = "Market Intelligence"

# 5. LOGICA VISTE
menu = st.session_state.menu

if menu == "My Watches":
    st.header("My Watches")
    cols = st.columns(3)
    for i in range(6):
        with cols[i % 3]:
            st.markdown(f"""
            <div class="watch-card">
                <div style="font-size: 18px; font-weight: 600;">All Dial Model {i+1}</div>
                <div style="color: #6B7280; font-size: 13px;">Ref: M001.431.11.0{i}1.02</div>
                <div style="font-size: 22px; font-weight: 700; color: #2E5BFF; margin-top: 10px;">€ {1200 + (i*150)}</div>
                <div style="font-size: 12px; color: #059669; font-weight: 600; margin-top: 5px;">● Up to date</div>
            </div>
            """, unsafe_allow_html=True)
            st.button("Set as Target", key=f"target_{i}")

elif menu == "Pricing Intelligence":
    st.header("Pricing Intelligence")
    st.plotly_chart(px.scatter(data, x='Price', y='PowerScore', color='Brand'), use_container_width=True)

elif menu == "Design Intelligence":
    st.header("Design Intelligence")
    st.plotly_chart(px.density_heatmap(data, x='Diameter', y='Thickness'), use_container_width=True)

elif menu == "Market Intelligence":
    st.header("Market Intelligence")
    st.line_chart(np.random.randint(42, 72, 12))
